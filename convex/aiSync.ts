...
export const getAIRecommendations = query({
  ...
  handler: async (ctx, args) => {
    ...
    const players = await ctx.db
      .query("players")
      .withIndex("by_game_status", (q) => q.eq("gameId", args.gameId).eq("status", "alive"))
      .collect()

    // Get recent player actions for pattern analysis (reserved for future use)
    // TODO: Integrate action patterns into AI recommendations
    void await ctx.db
      .query("playerActions")
        ...
        .map(async (player) => {
          const memory = await ctx.db
            ...
            .withIndex("by_shark_target", (q) =>
              q.eq("sharkUserId", sharkPlayer.userId).eq("targetUserId", player.userId)
            )
            .first()
    ...
    const targets = playerMemories.map((pm) => {
      ...
      if (pm.memory) {
        ...
        const hidePatterns = pm.memory.patterns.filter(
          (p) => p.type === "hiding_spot" && p.confidence > 0.6
        )
      ...
      const distance = Math.sqrt(
        (sharkPlayer.position.x - pm.currentPosition.x) ** 2 +
          (sharkPlayer.position.y - pm.currentPosition.y) ** 2 +
          (sharkPlayer.position.z - pm.currentPosition.z) ** 2
      )
...
export const getAISyncStatus = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      npcAwareness:
        activeNPCs.reduce((sum, npc) => sum + npc.awareness, 0) / (activeNPCs.length || 1),
    }
  },
})
