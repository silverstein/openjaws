...
export const getGrudgeLevels = query({
  ...
  handler: async (ctx, args) => {
    ...
    const grudges = memories.map(memory => {
      ...
      const memorableIntensity = memory.memorableMoments.reduce(
        (sum, moment) => sum + moment.intensity, 0
      ) / (memory.memorableMoments.length || 1);
      
      // Grudge level calculation
      let grudgeLevel = 0;
      if (memory.relationship === "nemesis") grudgeLevel = 10;
      else if (memory.relationship === "rival") grudgeLevel = 7;
      else if (memory.relationship === "respected") grudgeLevel = 5;
      else grudgeLevel = escapeRatio * 5 + memorableIntensity * 0.5;
...
export const getSharkMemoryStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const relationships = memories.reduce((acc, m) => {
    ...
    return {
      ...
      mostMemorableVictim: memories.sort((a, b) => 
        b.memorableMoments.length - a.memorableMoments.length
      )[0]?.targetUserId
    };
  }
});
