...
import { ABILITY_COOLDOWNS } from "./types"
      ...
      const nearbyPlayers = allPlayers
        ...
        .filter((p) => {
          const distance = Math.sqrt(
            (p.position.x - args.position.x) ** 2 + (p.position.y - args.position.y) ** 2
          )
      ...
      if (player.role === "swimmer" && game.sharkPlayerId) {
        ...
        if (shark && shark.status === "alive") {
          sharkDistance = Math.sqrt(
            (shark.position.x - args.position.x) ** 2 + (shark.position.y - args.position.y) ** 2
          )
        }
      }
...
export const useAbility = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const cooldown = player.archetype ? ABILITY_COOLDOWNS[player.archetype] : 30 // Default cooldown for shark
  ...
  handler: async (ctx, args) => {
    ...
    const existingMemory = await ctx.db
      ...
      .withIndex("by_shark_target", (q) =>
        q.eq("sharkUserId", shark.userId).eq("targetUserId", swimmer.userId)
      )
      .first()
