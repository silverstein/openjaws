import { freeTierConfig, APIUsageStats, getInitialUsageStats } from './config';
...
export function determineAIMode(): 'real' | 'mock' | 'cached' {
  ...
  if (cacheStats.sharkDecisions > 10 && cacheStats.avgQuality > freeTierConfig.MOCK_QUALITY_THRESHOLD) {
    // Randomly use cache to save API calls
    if (Math.random() < 0.3) { // 30% chance to use cache
      return 'cached';
    }
  }
