...
import { models, aiConfig } from './config';
import { generateMockSharkDecision, generateMockTaunt, streamMockSharkThoughts } from './mockResponses';
import { responseCache } from './responseCache';
import { determineAIMode, trackAPIUsage, getAPIUsageStats, updateCurrentMode } from './apiTracking';
...
- reasoning: tactical explanation for this decision`;
  ...
  try {
    ...
    const { text } = await generateText({
      ...
      maxTokens: aiConfig.maxTokens.shark,
    });
...
- Players nearby: ${context.currentPlayers.filter(p => {
    const distance = Math.sqrt(
      Math.pow(p.position.x - context.sharkPosition.x, 2) + 
      Math.pow(p.position.y - context.sharkPosition.y, 2)
    );
    return distance < 200;
  }).map(p => p.name).join(', ')}
...
export function analyzePlayerPattern(
  ...
  if (recentPositions.length < 3) return 'predictable';
  
  // Calculate movement variance
  const movements = recentPositions.slice(1).map((pos, i) => ({
    dx: pos.x - recentPositions[i].x,
    dy: pos.y - recentPositions[i].y,
    dt: pos.timestamp - recentPositions[i].timestamp,
  }));
  
  const avgSpeed = movements.reduce((sum, m) => 
    sum + Math.sqrt(m.dx * m.dx + m.dy * m.dy) / m.dt, 0
  ) / movements.length;
  
  const speedVariance = movements.reduce((sum, m) => {
    const speed = Math.sqrt(m.dx * m.dx + m.dy * m.dy) / m.dt;
    return sum + Math.pow(speed - avgSpeed, 2);
  }, 0) / movements.length;
  
  // Classify based on movement patterns
  if (speedVariance > 100) return 'erratic';
  if (avgSpeed > 5) return 'aggressive';
  if (avgSpeed < 2) return 'defensive';
  return 'predictable';
}

// Generate contextual taunts
export async function generateSharkTaunt(
  context: GameContext,
  trigger: string
): Promise<string> {
  const mode = determineAIMode();
  ...
  if (cached && Math.random() < 0.7) { // 70% chance to use cache
