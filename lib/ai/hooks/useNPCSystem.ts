...
export function useNPCSystem({
  ...
  const spawnNPC = useCallback((type: NPCType, position: { x: number; y: number }) => {
    ...
    return npc;
  }, [npcs, maxActiveNPCs, activeConversations, createNPC]);
  ...
  const getNearbyNPCs = useCallback((playerPosition: { x: number; y: number }): NPC[] => {
