...
import { generateMockNPCResponse, npcMockResponses } from './mockResponses';
...
If there's danger, balance your personality with appropriate concern.`;
  ...
  try {
    ...
    const stream = await streamText({
      ...
      maxTokens: aiConfig.maxTokens.npc,
    });
...
export function generateQuickNPCReaction(
  ...
  const reactions: Record<string, Record<NPCType, string[]>> = {
    shark_spotted: {
      ...
      lifeguard: ["SHARK! Everyone exit the water immediately!", "Clear the water! Shark sighting!"],
      ...
      scientist: ["Fascinating! Appears to be hunting behavior.", "Remarkable! Note the dorsal fin angle."],
    ...
    player_eaten: {
      ...
      scientist: ["Predation event confirmed. Tragic but natural.", "Classic ambush pattern. Unfortunate."],
    ...
    shark_defeated: {
      ...
      old_timer: ["Well I'll be... Haven't seen that since '87.", "You've earned your place in beach history."],
    },
    storm_coming: {
  ...
  const npcReactions = reactions[event][npcType];
