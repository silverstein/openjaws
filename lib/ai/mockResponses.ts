import { SharkPersonality, SharkDecision, SharkMemory, GameContext } from './sharkBrain';
...
export function generateMockSharkDecision(context: GameContext): SharkDecision {
  ...
  const actionResponses = responses.actions[analysis.suggestedAction] || responses.actions.hunt;
  
  const thought = thoughtOptions[Math.floor(Math.random() * thoughtOptions.length)];
  const reasoning = actionResponses[Math.floor(Math.random() * actionResponses.length)];
...
export function generateMockTaunt(context: GameContext, trigger: string): string {
  ...
  if (trigger === 'shark_damaged') {
    return tauntPatterns.sharkDamaged[
      Math.floor(Math.random() * tauntPatterns.sharkDamaged.length)
    ];
  }
...
export function generateMockNPCResponse(
  ...
  if (context === 'reaction' && trigger) {
    return npc.reactions[trigger as keyof typeof npc.reactions] || 
