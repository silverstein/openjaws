import { SharkDecision, GameContext, SharkPersonality } from './sharkBrain';
...
class ResponseCache {
  ...
  private hashContext(context: GameContext): string {
    const simplified = {
      ...
      avgPlayerHealth: Math.floor(
        context.currentPlayers.reduce((sum, p) => sum + p.health, 0) / 
        context.currentPlayers.length / 10
      ) * 10
    };
  ...
  private cleanupCache<T>(cache: CacheEntry<T>): CacheEntry<T> {
    ...
    if (now - cache.lastCleanup < 60000) { // Cleanup every minute max
      return cache;
    }
  ...
  getCachedSharkDecision(
    ...
    const selected = candidates.reduce((best, current) => {
      ...
      const currentScore = current.quality * 0.5 + currentSim * 0.3 + 
                          (1 - (now - current.timestamp) / this.CACHE_DURATION) * 0.2;
      const bestScore = best.quality * 0.5 + bestSim * 0.3 + 
    ...
    if (decision.targetPlayerId) {
      ...
      if (!targetExists) {
        ...
        const originalTarget = selected.response.targetPlayerId;
        const playersInWater = context.currentPlayers.filter(p => p.isInWater);
        if (playersInWater.length > 0) {
          decision.targetPlayerId = playersInWater[0].id;
          decision.destination = playersInWater[0].position;
        }
      }
    }
  ...
  getCachedNPCResponse(npcType: string, trigger: string): string | null {
    ...
    const validResponses = entry.responses.filter(r => 
      Date.now() - r.timestamp < this.CACHE_DURATION && r.quality > 0.5
    );
  ...
  getCachedTaunt(personality: SharkPersonality, situation: string): string | null {
    ...
    const validResponses = entry.responses.filter(r => 
      Date.now() - r.timestamp < this.CACHE_DURATION
    );
    ...
    for (let i = 0; i < validResponses.length; i++) {
      random -= weights[i];
      if (random <= 0) {
        validResponses[i].useCount++;
        return validResponses[i].response;
      }
    }
  ...
  updateResponseQuality(
    ...
    switch (type) {
      case 'shark':
        ...
        for (const [personality, entry] of this.sharkDecisionCache) {
          entries = entries.concat(entry.responses);
        }
        break;
      case 'npc':
        const npcEntry = this.npcResponseCache.get(key);
        if (npcEntry) entries = npcEntry.responses;
        break;
      case 'taunt':
        const tauntEntry = this.tauntCache.get(key);
        if (tauntEntry) entries = tauntEntry.responses;
        break;
    }
    
    // Find and update the response
    const response = entries.find(r => 
      JSON.stringify(r.response) === responseText ||
      r.response === responseText
    );
    
    if (response) {
      // Weighted average with existing quality
      response.quality = (response.quality * response.useCount + newQuality) / 
                        (response.useCount + 1);
    }
  }
