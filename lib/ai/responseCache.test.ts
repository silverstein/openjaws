...
describe("Response Cache System", () => {
  ...
  describe("Cache Cleanup", () => {
    ...
    it("should limit cache size", () => {
      ...
      // Add entries with same timestamp (all within cache duration)
      Date.now = () => baseTime

      // Add many responses to the SAME key (more than MAX_CACHE_SIZE)
      for (let i = 0; i < 150; i++) {
        responseCache.cacheTaunt("meta", "sameSituation", `Taunt ${i}`)
      }

      // Now advance time past cleanup interval and add one more to trigger cleanup
      Date.now = () => baseTime + 61000
      responseCache.cacheTaunt("meta", "sameSituation", "Final taunt")

      const stats = responseCache.getStats()
      // Should be limited to MAX_CACHE_SIZE (100) per key after cleanup
      // Cleanup happens after push, so the 151 entries get sliced to 100
