...
import { SharkPersonality, Position } from '@/convex/types'
import { AIDecision } from '../entities/Shark'
...
export class SharkAIController {
  ...
  private analyzeAndDecide(context: DecisionContext, recommendations: any): AIDecision {
    ...
    if (sharkState.rage > 80 && action !== 'retreat') {
  ...
  private getMemoryInfluence(targetUserId?: string): any {
    ...
    return {
      ...
      grudgeLevel: memory.relationship === 'nemesis' ? 1 : 
                   memory.relationship === 'rival' ? 0.5 : 0
    }
  }
    ...
    try {
      ...
      const gameContext = {
        ...
        nearbyPlayers: context.playerState ? [{
          ...
          isMoving: Math.abs(context.playerState.position.x) > 0.1 || 
