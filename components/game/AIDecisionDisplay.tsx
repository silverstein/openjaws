    ...
    return Array(segments)
      ...
      .map((_, i) => (
        <div
          key={`hunger-${i}`}
          className={`h-2 w-4 ${
            i < filledSegments ? "bg-orange-500" : "bg-gray-600"
    ...
    return Array(segments)
      ...
      .map((_, i) => (
        <div
          key={`rage-${i}`}
          className={`h-2 w-4 ${
            i < filledSegments ? "bg-red-500" : "bg-gray-600"
          } ${i === 0 ? "rounded-l" : ""} ${i === segments - 1 ? "rounded-r" : ""}`}
        />
      ))
  }
        ...
        {isVisible && (
          <>
            ...
            <div className="px-4 py-3 space-y-2">
              ...
              {patternRecognized && (
                <div className="mt-2 pt-2 border-t border-black/20">
                  <div className="text-yellow-300 text-xs font-medium">Pattern Detected!</div>
                  <div className="text-white/80 text-xs">
                    {patternRecognized.type} ({Math.round(patternRecognized.confidence * 100)}%
                    confidence)
                  </div>
                </div>
              )}
            ...
            {thoughtHistory.length > 1 && (
              <div className="px-4 py-2 bg-black/20 rounded-b-lg">
                ...
                {thoughtHistory.slice(1).map((thought, i) => (
                  <div key={`thought-${i}`} className="text-white/40 text-xs italic mt-1">
                    • {thought}
                  </div>
                ))}
              </div>
