...
export function VirtualJoystick({ onMove }: VirtualJoystickProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-32 h-32 rounded-full bg-black/30 backdrop-blur-sm border-2 border-white/20 touch-none"
      style={{
    ...
    >
      ...
      <div
        ref={stickRef}
        className={`absolute top-1/2 left-1/2 w-16 h-16 rounded-full bg-white/50 backdrop-blur-sm border-2 border-white/40 transform -translate-x-1/2 -translate-y-1/2 transition-all pointer-events-none ${
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-4 h-4 rounded-full bg-cyan-400/60" />
        </div>
      </div>
    </div>
  )
