...
import type { GameContext, SharkPersonality } from "@/lib/ai/sharkBrain"
import { AIShark } from "@/lib/game/entities/AIShark"
import { Player } from "@/lib/game/entities/Player"

type SharkMemoryEvent = "encounter" | "escape" | "damaged_shark" | "killed"
...
export function AIGameExample() {
  ...
  const sharkRef = useRef<AIShark | null>(null)
  const playersRef = useRef<Player[]>([])

  // Initialize AI hooks
  const { currentDecision, memories, updateMemory, startAutoDecisions, stopAutoDecisions } =
    useSharkAI({
  ...
  const { npcs, spawnNPC, triggerEvent } = useNPCSystem({
    maxActiveNPCs: 5,
  ...
  useEffect(() => {
    ...
    playersRef.current = [
      new Player(200, 200, "influencer", "player-1"),
      new Player(600, 200, "surferBro", "player-2"),
    ]
    ...
    if (sharkRef.current) {
      ...
      sharkRef.current.onMemoryUpdate = (playerId, playerName, event) => {
        updateMemory(playerId, playerName, event as SharkMemoryEvent)
      }
  ...
  const handlePlayerEaten = () => {
    ...
    if (sharkRef.current && playersRef.current[0]) {
      updateMemory(playersRef.current[0].id, playersRef.current[0].name || "Unknown", "killed")
    }
  }
  ...
  return (
    <div className="relative w-full h-screen bg-blue-100">
      ...
      <div className="absolute top-4 right-4 bg-white/90 p-4 rounded-lg shadow-lg z-50 space-y-4">
        ...
        <div>
          <label htmlFor="shark-personality" className="block text-sm font-medium mb-1">
            Shark Personality
          </label>
          <select
            id="shark-personality"
      ...
      <SharkThoughts
        ...
        gameContext={sharkRef.current?.getGameContext(playersRef.current) as GameContext | undefined}
      />
      ...
      <Commentary
        ...
        players={playersRef.current.map((p) => p.name)}
        sharkHealth={sharkRef.current?.getHealth() ?? 100}
        intensity={
          currentDecision?.action === "hunt"
            ? "intense"
            : currentDecision?.action === "stalk"
              ? "building"
              : currentDecision?.action === "ambush"
                ? "climactic"
                : "calm"
        }
      />
