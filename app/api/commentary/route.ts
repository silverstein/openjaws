...
import { models, aiConfig } from '@/lib/ai/config';
import { determineAIMode, trackAPIUsage, getAPIUsageStats, updateCurrentMode } from '@/lib/ai/apiTracking';
import { streamMockCommentary } from '@/lib/ai/mockResponses';
import { responseCache } from '@/lib/ai/responseCache';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const stylePrompts = {
      documentary: "You're David Attenborough narrating a nature documentary about sharks and humans.",
      sports: "You're an excited sports commentator calling the action like it's the championship finals.",
      horror: "You're the narrator of a suspenseful horror film, building dread and tension.",
      comedic: "You're a comedic narrator finding humor in the chaos while still respecting the danger.",
    };
      ...
      async start(controller) {
        try {
          ...
          if (mode === 'mock' || mode === 'cached') {
            ...
            for await (const chunk of streamMockCommentary(
              style as any,
              intensity as any,
              event
            )) {
          ...
          const response = streamText({
            ...
            maxTokens: aiConfig.maxTokens.commentary,
          });
