...
import { streamNPCResponse, generateNPCGreeting, generateQuickNPCReaction, type NPCContext } from '@/lib/ai/npcDialogue';
  ...
  try {
    ...
    if (!context || !message) {
      return NextResponse.json(
        { error: 'Message and context required' },
        { status: 400 }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'greeting') {
      ...
      if (!contextStr) {
        return NextResponse.json(
          { error: 'Context required for greeting' },
          { status: 400 }
        );
      }
    ...
    if (action === 'reaction') {
      ...
      if (!npcType || !event) {
        return NextResponse.json(
          { error: 'NPC type and event required' },
          { status: 400 }
        );
      }
    ...
    return NextResponse.json(
      { error: 'Failed to process NPC action' },
      { status: 500 }
    );
  }
}