...
import { makeSharkDecision, streamSharkThoughts, updateSharkMemory, generateSharkTaunt, getAPIUsageStats, type GameContext, type SharkMemory } from '@/lib/ai/sharkBrain';
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to process shark decision' },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to stream shark thoughts' },
      { status: 500 }
    );
  }
}