...
import { SharkMemory, MemoryPattern } from '@/convex/types'
...
export function useSharkMemory() {
  ...
  const loadMemories = useCallback(async (sharkUserId: string) => {
    setIsLoading(true)
    try {
      ...
      sharkMemories.forEach(memory => {
        ...
        if (memory.patterns.length > 0) {
          patternMap.set(memory.targetUserId, memory.patterns)
        }
      })
  ...
  const saveObservation = useCallback(async (
    ...
    try {
      ...
      if (currentMemory && pattern) {
        const updatedPatterns = [...(currentMemory.patterns || []), pattern]
  ...
  const getMemoryForPlayer = useCallback((userId: string): SharkMemory | undefined => {
    return memories.get(userId)
  }, [memories])
  
  // Get patterns for a specific player
  const getPatternsForPlayer = useCallback((userId: string): MemoryPattern[] => {
  ...
  const checkPattern = useCallback((
    userId: string,
    currentBehavior: { position: any; action: string }
  ): MemoryPattern | null => {
  ...
  const recordPatternAccuracy = useCallback(async (
    gameId: Id<"games">,
    sharkUserId: string,
    targetUserId: string,
...
function isNearHidingSpot(position: any, hidingSpotData: any): boolean {
  if (!hidingSpotData.location) return false
  
  const distance = Math.sqrt(
    Math.pow(position.x - hidingSpotData.location.x, 2) +
    Math.pow(position.y - hidingSpotData.location.y, 2)
  )
...
function matchesEscapeRoute(position: any, routeData: any): boolean {
  if (!routeData.path || routeData.path.length === 0) return false
